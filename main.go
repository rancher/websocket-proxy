...
func main() {
	...
	p := &proxy.Starter{
		...
		StatsPaths: []string{
			...
			"/v3/{hoststats:hoststats(\\/project)?(\\/)?}",
			"/v3/{containerstats:containerstats(\\/service)?(\\/)?}",
			"/v3/{containerstats:containerstats}/{containerid}",
		},
